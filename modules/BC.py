...
class Bert_CRF(nn.Module):
    def __init__(self, tag_to_ix, hidden_dim=768):
        ...
        # self.bigru = nn.GRU(bidirectional=True, num_layers=2, input_size=768, hidden_size=hidden_dim//2, batch_first=True)
        ...
        # self.multiheadAttention = nn.MultiheadAttention(hidden_dim, num_heads=1, dropout=0.0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None)
        # self.selfattention = SelfAttention(L=128,N=256,hidden_dim=768,num_heads=1)# L,N,hidden_dim,num_heads=1
    ...
    def _get_lstm_features(self, sentence):
        ...
        # 过gru
        # enc, _ = self.bigru(embeds)  # (seq_len, batch, input_size)--> (seq_len, batch,  hidden_size)

        # print("enc shape",enc.shape)
        # enc = self.attention(enc)
        # attn_output,_ = self.multiheadAttention(query=enc, key=enc, value=enc, key_padding_mask=None, need_weights=True, attn_mask=None)
