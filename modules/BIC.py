...
class BiGRU_CRF(nn.Module):
    def __init__(self, tag_to_ix, hidden_dim=768):
        ...
        self.embed = nn.Embedding(num_embeddings=21129, embedding_dim=768)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, tag_to_ix, hidden_dim=768):
        ...
        self.tagset_size = len(tag_to_ix)
        # self.hidden = self.init_hidden()
        self.bigru = nn.LSTM(bidirectional=True, num_layers=2, input_size=768, hidden_size=hidden_dim//2, batch_first=True)
        self.transitions = nn.Parameter(torch.randn(
            self.tagset_size, self.tagset_size
        ))
        self.embed = nn.Embedding(num_embeddings=21129, embedding_dim=768)
