...
class T5_BiGRU_CRF(nn.Module):
    def __init__(self, tag_to_ix, hidden_dim=768):
        super(T5_BiGRU_CRF, self).__init__()
        ...
        self.bert = T5Model.from_pretrained('t5-base')
    ...
    def _bert_enc(self, x):
        ...
        """
        with torch.no_grad():
#             print("this is electra len:",len(self.bert(x)))
#             encoded_layer, _  = self.bert(x)
            encoded_layer = self.bert(input_ids=x,  decoder_input_ids =x )
            enc = encoded_layer[-1]
        return enc
